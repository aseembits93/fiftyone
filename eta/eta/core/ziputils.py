...
def make_parallel_dirs(zip_path, ref_paths):
    ...
    """
    # Pre-extract base only once
    base = os.path.splitext(zip_path)[0]
    # Inline _get_basename_no_ext to avoid function call overhead for each ref_path
    join = os.path.join
    splitext = os.path.splitext
    basename = os.path.basename
    return [join(base, splitext(basename(p))[0]) for p in ref_paths]
...
def make_parallel_paths(zip_path, ref_paths, filename):
    ...
    """
    # Fuse the directory computation and join calls to avoid intermediate list
    base = os.path.splitext(zip_path)[0]
    join = os.path.join
    splitext = os.path.splitext
    basename = os.path.basename
    return [join(base, splitext(basename(p))[0], filename) for p in ref_paths]
