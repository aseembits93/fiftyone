...
def make_parallel_dirs(zip_path, ref_paths):
    ...
    """
    # Precompute base only once
    base = os.path.splitext(zip_path)[0]
    # Inline the basename-without-extension logic for better locality and avoid function call overheads
    join = os.path.join
    splitext = os.path.splitext
    basename = os.path.basename

    # Avoid function call for every path, and avoid unnecessary array allocation in listcomp
    return [
        join(base, splitext(basename(p))[0])
        for p in ref_paths
    ]
