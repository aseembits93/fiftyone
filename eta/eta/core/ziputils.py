...
def make_parallel_files(zip_path, ref_files):
    ...
    """
    base = os.path.splitext(zip_path)[0]
    # Precompute if base ends with separator to avoid '//'
    needs_sep = not base.endswith(os.sep)
    os_basename = os.path.basename
    if needs_sep:
        sep = os.sep
        # Fast string concatenation, avoids os.path.join overhead
        return [base + sep + os_basename(p) for p in ref_files]
    else:
        return [base + os_basename(p) for p in ref_files]
